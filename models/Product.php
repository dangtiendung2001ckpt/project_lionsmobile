...
class Product extends Delete
{
    ...
    public function getProductsCat($key, $ord, $limit, $offset)
    {
        $sql = "SELECT * FROM $this->_tableName where category_id ='$key' AND defaults = 1 ORDER BY $this->_primaryKey $ord LIMIT $limit OFFSET $offset  ";
        $this->execute($sql);
        $count = $this->numRows();
        $rows = [];
        if ((int)$count > 0) {
