...
class ChartpriceController extends DeleteValue {
    ...
    function homes(){
        ...
        $sl0= !empty($cat1['sum(order_detail.price)']) ? $cat1['sum(order_detail.price)'] : 0 ;
        $category1 ="['$name0', $sl0],";

        $name1 = $cat[1]['category_name'];
        $sl1= !empty($cat2['sum(order_detail.price)']) ? $cat2['sum(order_detail.price)'] : 0 ;
        $category2 ="['$name1', $sl1],";

        $name2 = $cat[2]['category_name'];
        $sl2= !empty($cat3['sum(order_detail.price)']) ? $cat3['sum(order_detail.price)'] : 0 ;
        $category3 ="['$name2', $sl2],";

        $name3 = $cat[3]['category_name'];
        $sl3= !empty($cat4['sum(order_detail.price)']) ? $cat4['sum(order_detail.price)'] : 0 ;
        $category4 ="['$name3', $sl3],";

        $name4 = $cat[4]['category_name'];
        $sl4= !empty($cat5['sum(order_detail.price)']) ? $cat5['sum(order_detail.price)'] : 0 ;
        $category5 ="['$name4', $sl4],";

        $name5 = $cat[5]['category_name'];
        $sl5= !empty($cat6['sum(order_detail.price)']) ? $cat6['sum(order_detail.price)'] : 0 ;
        $category6 ="['$name5', $sl5],";

        $name6 = $cat[6]['category_name'];
        $sl6= !empty($cat7['sum(order_detail.price)']) ? $cat7['sum(order_detail.price)'] : 0 ;
        $category7 ="['$name6', $sl6],";

        $name7 = $cat[7]['category_name'];
        $sl7= !empty($cat8['sum(order_detail.price)']) ? $cat8['sum(order_detail.price)'] : 0 ;
        $category8 ="['$name7', $sl7],";


        $name8= $cat[8]['category_name'];
        $sl8= !empty($cat9['sum(order_detail.price)']) ? $cat9['sum(order_detail.price)'] : 0 ;
        $category9 ="['$name8', $sl8],";

        $name9= $cat[9]['category_name'];
        $sl9= !empty($cat10['sum(order_detail.price)']) ? $cat10['sum(order_detail.price)'] : 0 ;
        $category10 ="['$name9', $sl9],";
        return $this->render('chartprice',$year,$category1,$category2,$category3,$category4,$category5,$category6,$category7,$category8,$category9,$category10,[]);
